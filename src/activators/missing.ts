...
export function missing<T>(activators: Activators<T>, fn: MissingKeyFn<T>=(t,k)=>{throw new Error(`'${k}' was undefined`)}): Activators<T> {
  return mapGetters<Activators<T>, Activators<T>>(activators, <K extends keyof T>(original: Activators<T>, result: Activators<T>, k: K): () => Activator<T, K> => {
    const originalActivator = original[k];
    const activator = (container: T) => {
      const value = originalActivator(container);
